...
int  SynopGMAC_Host_Interface_init(void)
{
	...
	TR("======000\n");
        if((retval = synopGMAC_init_network_interface("syn0",PHYS_TO_UNCACHED(0xc0000000000)))){
		TR("Could not initialize the Network interface.\n");
		return retval;
	}
        if((retval = synopGMAC_init_network_interface("syn1",PHYS_TO_UNCACHED(0xd0000000000)))){
		TR("Could not initialize the Network interface.\n");
		return retval;
	}
