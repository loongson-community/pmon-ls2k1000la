...
static u32 phy_read(synopGMACdevice * gmacdev, int offset)
{
	s32 status = -ESYNOPGMACNOERR;
	u32 data;
	status = synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, offset, &data);
	if(status < 0)
		return status;
	return data;
}
int genphy_aneg_done(synopGMACdevice * gmacdev)
{
	int retval = phy_read(gmacdev, MII_BMSR);
...
{	
	...
	status = synopGMAC_read_phy_reg(gmacdev->MacBase,gmacdev->PhyBase, MII_BMSR, &data);
	if(status)
		return status;

        if((data & BMSR_LSTATUS) == 0) {
		TR("No Link\n");
		gmacdev->LinkState0 = 0;
		...
		/* re-link detect net speed */
		if (gmacdev->LinkState0 != 0)
			return -ESYNOPGMACNOERR;

		if (genphy_aneg_done(gmacdev) != BMSR_ANEGCOMPLETE)
			return -ESYNOPGMACPHYERR;

		lpagb = phy_read(gmacdev, MII_STAT1000);
		if (lpagb < 0)
			return lpagb;

		adv = phy_read(gmacdev, MII_CTRL1000);
		if (adv < 0)
			return adv;

		if (lpagb & LPA_1000MSFAIL) {
			if (adv & CTL1000_ENABLE_MASTER)
				printf("Master/Slave resolution failed, maybe conflicting manual settings?\n");
			else
				printf("Master/Slave resolution failed\n");
			return -67;
		}
		common_adv_gb = lpagb & adv << 2;

		lpa = phy_read(gmacdev, MII_LPA);
		if (lpa < 0)
			return lpa;

		adv = phy_read(gmacdev, MII_ADVERTISE);
		if (adv < 0)
			return adv;
		...
		gmacdev->LinkState0 = 0x8 | ((gmacdev->Speed - 1) << 1) | (gmacdev->DuplexMode - 1);
	}
	if(gmacdev->Speed == SPEED1000)
...
/**
  ...
  * \return returns present tx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_get_tx_qptr(synopGMACdevice * gmacdev, u32 * Status, u_long * Buffer1, u32 * Length1, u_long * Data1, u_long * Buffer2, u32 * Length2, u_long * Data2 )
{
	u32  txover      = gmacdev->TxBusy;
	DmaDesc * txdesc = gmacdev->TxBusyDesc;
	int i;
...
//	gmacdev->TxBusyDesc = synopGMAC_is_last_tx_desc(gmacdev,txdesc) ? gmacdev->TxDesc : (txdesc + 1);
	...
	if(Buffer1 != 0)
		*Buffer1 = CACHED_MEMORY_ADDR | txdesc->buffer1;
	if(Length1 != 0)
		*Length1 = (txdesc->length & DescSize1Mask) >> DescSize1Shift;
	if(Data1 != 0)
		*Data1 = CACHED_MEMORY_ADDR | txdesc->data1;

	if(Buffer2 != 0)
		*Buffer2 = CACHED_MEMORY_ADDR | txdesc->buffer2;
	if(Length2 != 0)
		*Length2 = (txdesc->length & DescSize2Mask) >> DescSize2Shift;
	if(Data1 != 0)
		*Data2 = CACHED_MEMORY_ADDR | txdesc->data2;
...
/**
  ...
  * \return returns present rx descriptor index on success. Negative value if error.
  */
s32 synopGMAC_get_rx_qptr(synopGMACdevice * gmacdev, u32 * Status, u_long * Buffer1, u32 * Length1, u_long * Data1, u_long * Buffer2, u32 * Length2, u_long * Data2)
{
	u_long rxnext       = gmacdev->RxBusy;	// index of descriptor the DMA just completed. May be useful when data
						//is spread over multiple buffers/descriptors
	DmaDesc * rxdesc = gmacdev->RxBusyDesc;
...
//	return -1;
	...
	if(Length1 != 0)
		*Length1 = (rxdesc->length & DescSize1Mask) >> DescSize1Shift;
	if(Buffer1 != 0)
		*Buffer1 = CACHED_MEMORY_ADDR | rxdesc->buffer1;
	if(Data1 != 0)
		*Data1 = CACHED_MEMORY_ADDR | rxdesc->data1;
	if(Length2 != 0)
		*Length2 = (rxdesc->length & DescSize2Mask) >> DescSize2Shift;
	if(Buffer2 != 0)
		*Buffer2 = CACHED_MEMORY_ADDR | rxdesc->buffer2;
	if(Data1 != 0)
		*Data2 = CACHED_MEMORY_ADDR | rxdesc->data2;
