...
//u32 regbase = PHYS_TO_UNCACHED(0x1fe10000);	//sw:	for debug only
u64 regbase = PHYS_TO_UNCACHED(0x40050000);	// this is of no use in this driver! liyifu on 2010-01-12
...
static int yt8521_config_init(synopGMACdevice *gmacdev)
{
	u16 phyreg_val;
	u32 time = 0x10000;

	/* UTP */
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa000);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x0);

	/* set led0 link status when linkup led0 will bright */
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa00c);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x2600);

	/* set led1 Rx and Tx flashing*/
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa00d);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x1800);


	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa000);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0);

	/* disable auto sleep */
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x27);
	synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, &phyreg_val);
	phyreg_val &= ~(1 << 15);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x27);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, phyreg_val);

	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa000);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0);

	/* enable EXC clock when no wire plug */
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xc);
	synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, &phyreg_val);
	phyreg_val &= ~(1 << 12);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xc);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, phyreg_val);

	/* set Asymmetric_Pause and Pause value: 0*/
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x4, 0x11e1);

	/*phy_reset*/
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, 0xb140);

	do {
		synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, &phyreg_val);
	} while ((phyreg_val & 0x8000) && (time--));	//wait reset done
#if 0
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0x9e);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, 0x33);

	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa003);
	synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, &val0);
	val0 |= (0xd << 0 | 0xd << 10);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1e, 0xa003);
	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x1f, val0);

	synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, 0xb140);

	do {
		synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, &val0);
	} while(val0 & 0x8000);	//wait reset done
#endif
	return 0;
}
...
s32 synopGMAC_setup_tx_desc_queue(synopGMACdevice * gmacdev,u32 no_of_desc, u32 desc_mode)
{
	...
	if(first_desc == NULL){
		...
		first_desc = ((u_long)first_desc) & ~15;
...
s32 synopGMAC_setup_rx_desc_queue(synopGMACdevice * gmacdev,u32 no_of_desc, u32 desc_mode)
{
	...
	if(first_desc == NULL){
		...
		first_desc = (DmaDesc *)((u_long)first_desc & ~15);
...
#if SYNOP_TX_DEBUG
			printf("Finished Transmit at Tx Descriptor %d for skb 0x%llx and buffer = %llx whose status is %08x \n", desc_index,data1,dma_addr1,status);
#endif
#ifdef	IPC_OFFLOAD
	...
	do{
		...
		if(desc_index >= 0 && data1 != 0){
			...
			if(synopGMAC_is_rx_desc_valid(status)||SYNOP_PHY_LOOPBACK){
				...
				memset((u_long)data1,0,RX_BUF_SIZE);
...
//	if(dma_status_reg & GmacLineIntfIntr) {
		/* now use gmii mode so do'nt check dma_status */
...
//		dumpreg(regbase);
        ...
        if(interrupt & synopGMACDmaRxStopped){
			...
			adapter->synopGMACNetStats.rx_over_errors++;
			do{
				u_long skb = (u_long)plat_alloc_memory(RX_BUF_SIZE);		//should skb aligned here?
				if(skb == NULL){
...
//	DmaDesc * dbgdesc;	//sw: dbg
	...
	//s32 reserve_len=2;
	u64 dma_addr;
	u_long skb;	//sw	we just use the name skb in pomn
	u_long skb1;
...
//	dumpreg(regbase);
	...
	do{
		skb = (u_long)plat_alloc_memory(RX_BUF_SIZE);		//should skb aligned here?
		if(skb == NULL){
			TR0("ERROR in skb buffer allocation\n");
			break;
//			return -ESYNOPGMACNOMEM;
...
//	synopGMAC_rx_enable(gmacdev);	
	...
	printf("register poll interrupt: gmac 0\n");
...
s32 synopGMAC_linux_xmit_frames(struct ifnet* ifp)
{
	...
	while(ifp->if_snd.ifq_head != NULL){
		if(!synopGMAC_is_desc_owned_by_dma(gmacdev->TxNextDesc))
		{
			...
			bf1 = (u_long)plat_alloc_memory(TX_BUF_SIZE);
			if(bf1 == 0)
				return -1;
			...
			bf2  = CACHED_TO_UNCACHED(bf1);	
			dma_addr  = UNCACHED_TO_PHYS(bf2);
#ifndef LOONGSON_2G5536
			dma_addr &= 0x0fffffff;
#endif
...
int init_phy(synopGMACdevice *gmacdev)
{
	...
	printf(" phy device ID is: %x, ", data2);
	...
	if(data2 == 0x141 && ((data3 >> 10) == 0x3)){ //marvel ethernet phy
	...
	} else if ((data2 == 0x6) && (data3 == 0x1ce0)) {
		u16 data, data1;
		printf("gmac phy: C28SOI_GNETPHY_4v0\n");
		printf("set to Auto-Negotiation 1000M\n");
		synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, 0x1140);
		synopGMAC_write_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, 0x9140);
		do {
			synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 0x0, &data);
		} while(data & 0x8000);	//wait reset done
		synopGMAC_read_phy_reg(gmacdev->MacBase, gmacdev->PhyBase, 1, &data);
	} else if ((data2 == 0x0) && (data3 == 0x11a)) {
		printf("gmac phy: YT8521 recognized\n");
		yt8521_config_init(gmacdev);
		printf("==== YT8521 phy reset ok.\n");
	} else if((data2 == 0x84b9)&&(data3 == 0x5011)){
#if 0
		u16 data;
		synopGMAC_read_phy_reg(gmacdev->MacBase,(gmacdev->PhyBase + 1),22,&data);
		data &= 0xfffb;
		synopGMAC_write_phy_reg(gmacdev->MacBase,gmacdev->PhyBase + 1,22,data);
		synopGMAC_read_phy_reg(gmacdev->MacBase,(gmacdev->PhyBase + 1),22,&data);
#endif	
    } else {
        u16 data,err;
		printf("==== Warning: unrecoganized gmac phy!\n");
          ...
          //data = data | 0x8000;
          //err = synopGMAC_write_phy_reg((u64 *)gmacdev->MacBase,gmacdev->PhyBase,0x0,data);
          err = synopGMAC_read_phy_reg((u64 *)gmacdev->MacBase,gmacdev->PhyBase,0x11, &data);
	        printf(" phy 0x10 reg is: %x\n", data);
...
s32  synopGMAC_init_network_interface(char* xname, u64 synopGMACMappedAddr)
{
	...
	mac_addr0[5] += gmac_num;
#else
	i2c_init();//configure the i2c freq
	if(mac_read(eeprom_addr, mac_addr0, 6) == 0)
        mac_addr0[5] += gmac_num;
#endif
#endif
	...
	TR("0x900000001fe10040:%x\n",synopGMACReadReg(0x900000001fe10040,0));
	TR("0x900000001fe10040:%x\n",synopGMACReadReg(0x900000001fe10040,0));
#endif
