...
static struct nand_ecclayout spinand_oob_256 = {
	.eccbytes = 128,
	.eccpos = {
...
static int spinand_read_id(struct spinand_info *info, u8 *id)
{
	...
	if(nand_id[0] == 0xc8 && (nand_id[1] == 0xb4 || nand_id[1] == 0xa4)) {
		info->gd_ctype = 1;
		id[0] = nand_id[0];
		id[1] = nand_id[1];
	} else {
...
#ifdef NOUSED_MTD_SPINAND_ONDIEECC
	unsigned int i, j;
    unsigned int buf_size = info->writesize + info->oobsize;
    unsigned int offset_end = offset + len;

	enable_read_hw_ecc = 0;
	wbuf = devm_kzalloc(&info->spi->dev, buf_size, GFP_KERNEL);
	spinand_read_page(info, page_id, 0, buf_size, wbuf);

	for (i = offset, j = 0; i < info->writesize; i++, j++)
		wbuf[i] &= buf[j];

    for (i = info->writesize;i < offset_end;i++,j++)
        wbuf[i+2] &= buf[j];
...
int spinand_probe(struct spi_device *spi_nand)
{
	...
	chip->ecc.layout = &spinand_oob_256;
