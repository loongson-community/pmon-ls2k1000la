...
int nand_generic_write(struct mtd_info *mtd,
		uint8_t *buf, int len, int offset,uint8_t *oob, int ooblen, int ooboffs, int ecc)
{
	struct nand_chip *chip = mtd->priv;
	uint8_t *wbuf;
	wbuf = buf;
	...
	if(offset || len != mtd->writesize)
	{
		...
		memcpy(&chip->buffers.databuf[offset], buf, len);
		wbuf = chip->buffers.databuf;
	}
	memset(chip->oob_poi, 0xff, mtd->oobsize);
	if(oob && ooblen)
	{
	 memcpy(chip->oob_poi + ooboffs, oob, ooblen);
	}

	if(ecc)
		nand_write_page_swecc(mtd, chip, wbuf);
	else
		nand_write_page_raw(mtd, chip, wbuf);
	return 0;
}

int nand_generic_read(struct mtd_info *mtd,
		uint8_t *buf, int len, int offset,uint8_t *oob, int ooblen, int ooboffs, int ecc)
{
	struct nand_chip *chip = mtd->priv;
	int aligned = (offset == 0 && len == mtd->writesize);
	uint8_t *bufpoi = aligned ? buf : chip->buffers.databuf;
	chip->oob_poi = chip->buffers.oobrbuf;
	if(ecc)
		nand_read_page_swecc(mtd, chip, bufpoi);
	else
		nand_read_page_raw(mtd, chip, bufpoi);
	if (!aligned) 
		memcpy(buf, chip->buffers.databuf + offset, len);
	if(oob && ooblen)
		memcpy(oob, chip->oob_poi + ooboffs, ooblen);
	return 0;
}
