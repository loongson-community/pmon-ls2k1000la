...
{
	unsigned int mode = readl(sm502_reg_base + 0x0054);
	unsigned int clock = readl(sm502_reg_base + 0x003c);
	unsigned char reg = 0;
	unsigned int sm501_freq, pllclock;
...
extern struct usb_device *usb_alloc_new_device(void *hc_private);
	...
	|| (defined LOONGSON_2G1A) || (defined LOONGSON_2F1A) || (defined LOONGARCH_2K1000)
...
SM502_HC:
	...
	if (PCI_VENDOR(pa->pa_id) == 0x14 && PCI_PRODUCT(pa->pa_id) == 0x7a24) {
		/* this version registers need fixup address */
		if (PCI_REVISION(pa->pa_class) == 2)
			ohci->sc_sh += 0x1000;
	}
	printf("usb base addr : 0x%llx, bus_base is : 0x%lx\n", ohci->sc_sh,
			memt->bus_base);
...
#else
				((ed_t *)
				CACHED_TO_UNCACHED(*((u32 *) & ed->hwNextED)))->ed_prev =
					ed->ed_prev;
#endif
			}
		}
		...
		void *retbuf)
{
	...
	td->retbuf = retbuf;
#ifdef DEBUG
	printf(" %s %d  -> %llx\n", __func__, __LINE__, td->ed);
	printf(" %s %d  -> %llx\n", __func__, __LINE__, td->next_dl_td);
	printf(" %s %d  -> %x  \n", __func__, __LINE__, td->index);
	printf(" %s %d  -> %llx\n", __func__, __LINE__, td->data);
	printf(" %s %d  -> %lx \n", __func__, __LINE__, td->transfer_len);
	printf(" %s %d  -> %llx\n", __func__, __LINE__, td->retbuf);
#endif
	if (!len)
		data = 0;
...
int usb_lowlevel_init(ohci_t * gohci)
{
	...
	wait_ms(1);
#endif
	printf("OHCI %llx initialized ok\n", gohci);
	return 0;
