	...
	void *data, unsigned short size, int timeout)
{
	...
	while (timeout--) {
		if (!((volatile unsigned int)dev->status & USB_ST_NOT_PROC))
			break;
		wait_ms(1);
	}
 ...
 *	  [IN] pipe: describe the property of a pipe,more details about it,
 *		      please see the usb.h.
 *	  [IN] data: an all-purpose pointer to the data that would be returne
 *		      through usb channel.
...
int usb_parse_config(struct usb_device *dev, unsigned char *buffer, int cfgno)
{
	...
	while (index + 1 < dev->config.wTotalLength) {
		...
		case USB_DT_INTERFACE:
			...
			if (ifno && dev->config.if_desc[ifno-1].bInterfaceNumber == ((struct usb_interface_descriptor *)&buffer[index])->bInterfaceNumber) {					ifno--;
				ifno--;
			}
			else
...
{
	unsigned char mybuf[USB_BUFSIZ] = {0};
	unsigned char *tbuf;
	int err;
	unsigned int u, idx;
...
#undef NEW_INIT_SEQ
	...
	if (dev->speed == USB_SPEED_LOW || dev->speed == USB_SPEED_UNKNOWN) {
