...
//-----------------------------------------------------------------------
extern int ticks;
inline int lock_fdc(int drive, int flags)
{
	return  0;
}
#define unlock_fdc(...)
...
{
	volatile unsigned long counts = loops;
	while (counts--);
}
...
extern __inline__ void __udelay(unsigned long usecs, unsigned long lpj)
{
	...
	usecs *= (unsigned long)(((0x8000000000000000ULL / (500000 / HZ)) +
				   0x80000000ULL) >> 32);
	/*this code similar with assemble code*/
	usecs = usecs * lpj;
	usecs >>= 32;
 ...
 *
 * On LOONGARCH using vdma, this actually means that *all* transfers go thru
 * the * track buffer since 0x1000000 is always smaller than KSEG0/1.
 * Actually this needs to be a bit more complicated since the so much different
 * hardware available with LOONGARCH CPUs ...
 */
...
void *tgt_poll_register(int level, int(*func) __P((void *)), void *arg);
