...
static __le16 nvme_get_cmd_id(int dev_num)
{
	static unsigned short cmdid[MAX_NVME];
	if (dev_num >= MAX_NVME) {
		printf("ERROR: %s %d dev_num is too large !\n", __func__, __LINE__);
		while(1);
	}

	return cpu_to_le16((cmdid[dev_num] < USHRT_MAX) ? cmdid[dev_num]++ : 0);
}
...
static void nvme_submit_cmd(struct nvme_queue *nvmeq, struct nvme_command *cmd)
{
	...
	int i;
	printf("\ncmd \n");
	for (i = 0;i < sizeof(*cmd); i += 4) {
		printf("%x ", readl((ulong)cmd + i));
		if (!((i + 4) % 16))
			printf("\n");
	}
#endif
...
static int nvme_submit_sync_cmd(struct nvme_dev *ndev, struct nvme_queue *nvmeq,
				...
				u32 *result, unsigned timeout)
{
	...
	cmd->common.command_id = nvme_get_cmd_id(ndev->instance);
...
static int nvme_submit_admin_cmd(struct nvme_dev *dev, struct nvme_command *cmd,
				 u32 *result)
{
	return nvme_submit_sync_cmd(dev, dev->queues[NVME_ADMIN_Q], cmd,
...
static int nvme_blk_probe(struct nvme_blk_t *udev)
{
	...
	ns->ns_id = 1; /* ns_id default value is 1 */
			 ...
			 lbaint_t blkcnt, void *buffer, bool read)
{
	...
	while (total_lbas) {
		...
		status = nvme_submit_sync_cmd(dev, dev->queues[NVME_IO_Q],
				&c, NULL, IO_TIMEOUT);
		if (status)
			break;
