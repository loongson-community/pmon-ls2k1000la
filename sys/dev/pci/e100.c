...
static struct sk_buff *dev_alloc_skb(unsigned int length) {
	...
	if (!skb || !skb->data) {
		printf("not enough memory!enlarge NKMEMCLUSTERS in sys/arch/mips/include/param.h");
	}
...
static long pci_resource_end(struct pci_dev *pdev, int bar)
{
	int reg = PCI_MAP_REG_START + bar * 4;
	int rv, size, flags, type;
	unsigned long start;
...
static int fxp_match(parent, match, aux)
...
static void fxp_attach(parent, self, aux)
...
static int fxp_init(struct net_device *netdev)
{
	struct ifnet *ifp = &netdev->arpcom.ac_if;
	int stat=0;
	if (!netdev->opencount) {
		stat = e100_open(netdev);
		netdev->opencount++;
	}
...
static int fxp_stop(struct net_device *netdev)
{
	struct ifnet *ifp = &netdev->arpcom.ac_if;
	ifp->if_timer = 0;
...
static int fxp_ether_ioctl(ifp, cmd, data)
	struct ifnet *ifp;
