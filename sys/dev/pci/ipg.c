...
static long pci_resource_start(struct pci_dev *pdev, int bar)
{
	int reg = PCI_MAP_REG_START + bar * 4;
	int rv, size, flags, type;
	unsigned long start;
...
{
	IPG_WRITE_PHYCTRL(baseaddr, IPG_PC_MGMTCLK_LO | (IPG_PC_MGMTDATA & 0) |
...
static inline int ipg_nic_rx__CheckFrameSEType(struct net_device *ipg_ethernet_device)
{
	...
	if ((sp->RFDList[sp->CurrentRFD].RFS) & IPG_RFS_FRAMESTART)
		FoundStartEnd += 1;
	if ((sp->RFDList[sp->CurrentRFD].RFS) & IPG_RFS_FRAMEEND)
		FoundStartEnd += 10;
...
static int ipg_nic_rx(ipg_device_type *ipg_ethernet_device)
{
	...
	while ((sp->rfdlist[sp->currentrfd].rfs) & ipg_rfs_rfddone) {
		...
		switch (ipg_nic_rx__checkframesetype(ipg_ethernet_device)) { // frame in one rfd
			//-----------
...
static int ipg_match(parent, match, aux)
...
static void ipg_attach(parent, self, aux)
...
static int ipg_init(struct net_device *netdev)
{
	struct ifnet *ifp = &netdev->arpcom.ac_if;
	int stat = 0;
	ifp->if_flags |= IFF_RUNNING;
	if (!netdev->opencount) {
		stat = netdev_open(netdev);
		netdev->opencount++;
	}
	return stat;
}

static int ipg_stop(struct net_device *netdev)
{
	struct ifnet *ifp = &netdev->arpcom.ac_if;
	ifp->if_timer = 0;
...
static int ipg_ether_ioctl(ifp, cmd, data)
	struct ifnet *ifp;
