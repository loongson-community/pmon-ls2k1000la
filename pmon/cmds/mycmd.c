...
unsigned int syscall_addrtype = 1;
...
unsigned long long str2addr(const char *nptr, char **endptr, int base)
{
	...
	if (syscall_addrtype % 4 == 0) {
		...
		if (syscall_addrtype % 4 == 3)
			result |= UNCACHED_MEMORY_ADDR;
		else if (syscall_addrtype % 4 == 2)
			result |= CACHED_MEMORY_ADDR;
		else if ((result & UNCACHED_MEMORY_ADDR) != UNCACHED_MEMORY_ADDR)
			result |= UNCACHED_MEMORY_ADDR;
	}
	return result;
}
...
static int __disksyscall1(int type, long long addr, union commondata *mydata)
{
	...
	{
		case 1:
			fread(&mydata->data1, 1, 1, fp);
			break;
		case 2:
			fread(&mydata->data2, 2, 1, fp);
			break;
		case 4:
			fread(&mydata->data4, 4, 1, fp);
			break;
		case 8:
			fread(&mydata->data8, 8, 1, fp);
			break;
	}
	fclose(fp);
	return 0;
...
static int __disksyscall2(int type, long long addr, union commondata *mydata)
{
	...
	{
		case 1:
			fwrite(&mydata->data1, 1, 1, fp);
			break;
		case 2:
			fwrite(&mydata->data2, 2, 1, fp);
			break;
		case 4:
			fwrite(&mydata->data4, 4, 1, fp);
			break;
		case 8:
			fwrite(&mydata->data8, 8, 1, fp);
			break;
	}
	fclose(fp);
	return 0;
...
static int __pcisyscall1(int type, unsigned long long addr, union commondata *mydata)
{
	...
	{
		case 1 :
			mydata->data1 = _pci_conf_readn(mytag, addr, 1);
			break;
		case 2 :
			mydata->data2 = _pci_conf_readn(mytag, addr, 2);
			break;
		case 4 :
			mydata->data4 = _pci_conf_readn(mytag, addr, 4);
			break;
		case 8 :
			mydata->data8[0] = _pci_conf_readn(mytag, addr, 4);
			mydata->data8[1] = _pci_conf_readn(mytag, addr + 4, 4);
			break;
	}
	return 0;
}
...
static int __pcisyscall2(int type, unsigned long long addr, union commondata *mydata)
{
	...
	{
		case 1:
			_pci_conf_writen(mytag, addr, mydata->data1, 1);
			return 0;
		case 2:
			_pci_conf_writen(mytag, addr, mydata->data2, 2);
			return 0;
		case 4:
			_pci_conf_writen(mytag, addr, mydata->data4, 4);
			return 0;
		case 8:
			break;
	}
...
static int __syscall1(int type, long long a, union commondata *mydata)
{
	...
	{
		case 1 :
			mydata->data1 = *(volatile char *)addr;
			break;
		case 2 :
			mydata->data2 = *(volatile short *)addr;
			break;
		case 4 :
			mydata->data4 = *(volatile int *)addr;
			break;
		case 8 :
			*(unsigned long long *)mydata->data8 = *(volatile long long *)addr;
			break;
	}

//        printf("The function is ____syscall1\r\n");
...
static int __syscall2(int type, long long a, union commondata *mydata)
{
	...
	{
		case 1 :
			*(volatile char *)addr = mydata->data1;
			break;
		case 2 :
			*(volatile short *)addr = mydata->data2;
			break;
		case 4 :
			*(volatile int *)addr = mydata->data4;
			break;
		case 8 :
			*(volatile long long *)addr = *(unsigned long long *)mydata->data8;
			break;
	}

//        printf("The function is ____syscall2\r\n");
...
static int dump(int argc, char **argv)
{
	...
	for (j = 0; j < count; j = j + 16 / type, addr = addr + 16 / syscall_addrwidth) {
		...
		for (i = 0; type * i < 16; i++) {
			...
			{
				case 1 :
					nr_printf("%02x ", pmydata->data1);
					break;
				case 2 :
					nr_printf("%04x ", pmydata->data2);
					break;
				case 4 :
					nr_printf("%08x ", pmydata->data4);
					break;
				case 8 :
					nr_printf("%08x%08x ", pmydata->data8[1], pmydata->data8[0]);
					break;
			}
			if (j + i + 1 >= count) {
				int k;
...
static int modify(int argc, char **argv)
{
	...
	while (1) {
		...
		{
			case 1 :
				nr_printf("%02x ", mydata.data1);
				break;
			case 2 :
				nr_printf("%04x ", mydata.data2);
				break;
			case 4 :
				nr_printf("%08x ", mydata.data4);
				break;
			case 8 :
				nr_printf("%08x%08x ", mydata.data8[1], mydata.data8[0]);
				break;
		}
		memset(str, 0, 100);
		nr_gets(str);
...
static int checksum(int argc, char **argv)
{
	...
	for (idx = 0; idx < count; idx++) {
		...
		*/
		if (quiet < 1)
			printf("%d : checksuming  0x%lx, size = 0x%lx\n", idx, sum, total);
...
{
	int i, n, type_counts, status;
	int off = 0x400;
	unsigned char g_type[16];
	unsigned long g_startsec[1];
	unsigned long g_endsec[1];
	struct partition *p0;
	DeviceDisk *p;
	FILE *fp;
	char device[0x40], *dev_name;
	int buf[0x10];
	unsigned char gbuf[48];
	int len;
	...
	printf("\n%-10s%-6s%-12s%-12s%-12s%-10s%-32s\n","Device", "Boot", "Start", "End", "Sectors", "Id", "System");
	p = FindDevice(dev_name);
	if (p == NULL) {
		fclose(fp);
		return -1;
	}

	dev_name = strlen(dev_name) < 8 ? dev_name : dev_name + 8;
	/* GPT disk */
	if ((buf[1] & 0xff) == 0xee) {
		len = p->dev_fscount;
		for (n = 'a'; len > 0; off += 0x80, len--) {
			fseek(fp, off, 0);
			fread(gbuf, 48, 1, fp);
			memcpy(g_type, gbuf, 16);
			g_startsec[0] = *(unsigned long *)(gbuf + 32);
			g_endsec[0] = *(unsigned long *)(gbuf + 40);
			for (i = 1; i < sizeof(gpt_type)/sizeof(struct gpt_parttype); i++) {
				if (!memcmp(g_type, gpt_type[i].type, sizeof(g_type))) {
					sprintf(device, "%s%c", dev_name, n++);
					printf("%-10s", device);
					printf("%-6c", (i <= 2) ? '*' : ' ');
					printf("%-12d", g_startsec[0]);
					printf("%-12d", g_endsec[0]);
					printf("%-12d", g_endsec[0] - g_startsec[0] + 1);
					printf("%08X", *(unsigned long *)g_type);
					printf("  %-32s\n", gpt_type[i].name);
					break;
				}
			}
	...
	/* MBR disk */
		...
		for (n = 0, p0 = (void *)buf; n < 4; n++, p0++) {
			...
			for (i = 0; i < type_counts; i++)
				if (known_parttype[i].type == p0->sys_ind)
					break;

			sprintf(device, "%s%c", dev_name, 'a' + n);
			printf("%-10s%-6c%-12d%-12d%-12d%-8X  %-32s\n", device, (p0->boot_ind == 0x80) ? '*' : ' ',	\
					p0->start_sect, p0->start_sect + p0->nr_sects, p0->nr_sects,	\
					p0->sys_ind, i<type_counts ? known_parttype[i].name : "unknown");
...
static int cmd_ifconfig(int argc, char **argv)
{
	...
	if (argc == 2) {
		...
		for (i = 0; i < sizeof(cmds) / sizeof(char *); i++)
			if (!strncmp(argv[2], cmds[i], strlen(cmds[i])))
				break;
...
//yzk add for test
static int cmd_mymalloc(int argc, char **argv)
{
	void *p;
	long long val;
	if (argc != 2)
		return -1;
	val = nr_strtoll(argv[1], 0, 0);
	p = malloc(val);
...
//yzk add for test
static int cmd_mymemset(int argc, char **argv)
{
	long long addr, count;
	char c;
	int ret;
	if (argc != 4)
		return -1;
	addr = nr_str2addr(argv[1], 0, 0);
	c = nr_strtol(argv[2], 0, 0);
	count = nr_strtoll(argv[3], 0, 0);
	ret = highmemset(addr, c, count);
	return ret;
}
//yzk add for test
static int cmd_mysyscall1(int argc, char **argv)
{
	int type, ret;
	long long a;
	union commondata *mydata;
	if (argc != 4)
		return -1;
	type = nr_strtol(argv[1], 0, 0);
	a = nr_str2addr(argv[2], 0, 0);
	mydata = nr_str2addr(argv[3], 0, 0);
	ret = __syscall1(type, a, mydata);
	return ret;
}
...
static const Cmd Cmds[] =
{
	...
	{"test_malloc", "input size", 0, "malloc a address", cmd_mymalloc, 0, 99, CMD_REPEAT},
	{"test_memset", "input count", 0, "memset a address", cmd_mymemset, 0, 99, CMD_REPEAT},
	{"test_syscall1", "input syscall1 value", 0, "syscall1", cmd_mysyscall1, 0, 99, CMD_REPEAT},
	{0, 0}
};
