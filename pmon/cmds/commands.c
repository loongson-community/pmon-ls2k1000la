     ...
     char           *av[];
{
    ...
    if (!get_rsa_reg (&from, av[1]) || !get_rsa_reg (&to, av[2]))
	return (-1);
     ...
     char           *av[];
{
    ...
    if (!get_rsa_reg (&from, av[1]) || !get_rsa_reg (&to, av[2]) || !get_rsa_reg (&n, av[3]))
	return (-1);
    if (to < from)
	while (n-- > 0)
     ...
     char           *av[];
{
    ...
    /* get <addr> */
    if (optind >= ac || !get_rsa_reg (&adr, av[optind++]))
      return (-1);
