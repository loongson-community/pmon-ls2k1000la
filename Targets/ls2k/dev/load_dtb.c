...
#include <efi_api.h>
static const efi_guid_t dtb_guid = EFI_FDT_GUID;
...
#include "target/dtsnode.h"

void nodecfgs(char * head)
{
	int nodeoffset;
	int id, len;
	pcitag_t tag;
	...
	unsigned int val;
	const void *nodep;	/* property node pointer */
	for (id = 0; id < sizeof(nodes) / sizeof(nodes[0]); id++) {
		nodeoffset = fdt_path_offset(head, nodes[id].name);
		if (nodeoffset < 0)
			continue;

		nodep = fdt_getprop (head, nodeoffset, (const char* )"reg", &len);
		if (len <= 0)
			continue;

		memcpy(data, nodep, len);
		tag = _pci_make_tag(nodes[id].bus, nodes[id].dev, nodes[id].func);
	    val = _pci_conf_read32(tag, 0x00);

		if (val != 0xffffffff) {
			val =_pci_conf_read32(tag, 0x10);
			val &= ~0xf;
			data[1] = cpu_to_be32(val);
			printf("Dev %d reg base changing to 0x%x\n", nodes[id].dev, val);
		}

		if (fdt_setprop(head, nodeoffset, "reg", data, len))
		{
 			printf("Set reg base error\n");
		}
	}
}
...
//	tgt_flashprogram((char *)working_fdt - 4, DTB_SIZE, buff, 0);
	if(!check_mac_ok() || !check_pci_bridge_ok() || !check_mem_args(working_fdt)) {
		u8 buff[DTB_SIZE] = {0};
		fdt_open_into(working_fdt, buff + 4, DTB_SIZE - 8);
		update_mac(buff + 4, 0);
		update_mac(buff + 4, 1);
...
static int set_mem_xbar()
{
	...
	/*
	   ...
	   0x900000001fe10020 is old higmem window.
	   0x900000001fe10018 is a free window, use it remap highmem window to 2G.
	 */
	for(i=LS2K1000_XBAR_WIN3_BASE;i!=LS2K1000_XBAR_WIN0_MASK;i+=8) {
		if (*(volatile long long *)(i+0x80) == 0)  {
			*(volatile long long *)i = 0x80000000ULL;
			*(volatile long long *)(i+0x40) = (*(volatile long long *)LS2K1000_XBAR_WIN4_MASK)|0xffffffff80000000ULL;
			*(volatile long long *)(i+0x80) = *(volatile long long *)LS2K1000_XBAR_WIN4_MMAP;
			break;
		}
	}
	configured = 1;
...
int update_bootarg(int ac, char ** av, void * ssp)
{
	int i, len, err;
...
unsigned long long setup_dtb(int ac, char ** av)
{
	...
	efi_install_configuration_table(&dtb_guid, (void *)VA_TO_PHYS(ssp));
...
static const Cmd Cmds[] =
{
	...
	{"save_dtb","",0,"save dtb file to flash", save_dtb,0,99,CMD_REPEAT},
	{0, 0}
};
