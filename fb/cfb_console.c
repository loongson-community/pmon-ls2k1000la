...
#define CONSOLE_ROW_LAST	(video_console_address + (CONSOLE_ROWS - 1 + video_logo_height / VIDEO_FONT_HEIGHT) * CONSOLE_ROW_SIZE)
 ...
 * Only uncompressed
 */
int video_display_bitmap (void* bmp_image, int x, int y)
{
	...
	/* for align */
	if(!video_logo_width)
		video_logo_width = width / VIDEO_FONT_WIDTH * VIDEO_FONT_WIDTH;
	if(!video_logo_height)
		video_logo_height = height / VIDEO_FONT_HEIGHT * VIDEO_FONT_HEIGHT;

	bpp = le16_to_cpu (bmp->header.bit_count);
	colors = le32_to_cpu (bmp->header.colors_used);
	compression = le32_to_cpu (bmp->header.compression);

	//tgt_printf("Display-bmp: %d x %d  with %d colors\n", width, height, colors);
	...
	if (x == AT_END) {
		x = VIDEO_COLS - width;	//Place the logo at end
	} else if (x == AT_MID) {
		x = (VIDEO_COLS - width) >> 1;	//Place the logo at middle
	}
	if(y == AT_END) {
		y = VIDEO_ROWS - height;	//Place the logo at end
	} else if (y == AT_MID) {
		y = (VIDEO_ROWS - height) >> 1;	//Place the logo at middle
	}
...
#ifdef PROGRESS_BAR
	if (video_display_bitmap (logo_data, AT_MID, AT_MID) == 0)
#else
	if (video_display_bitmap (logo_data, AT_END, 0) == 0)
#endif
	{
...
#define FB_YSIZE 600
...
#if   defined(LOONGSON3A5000)
	if(fbbase < 0x20000000)
	//	pGD->frameAdrs = 0xb0000000 | fbbase;
		pGD->frameAdrs = PHYS_TO_UNCACHED(0x10000000) | fbbase;
	else
	...
	pGD->frameAdrs = fbbase;
#endif	
	printf("cfb_console init,fb=%llx\n",pGD->frameAdrs);
...
#define LOG_CNT		20	//The minimum of times print_logo_tick() called, count the drivers of the devices include in "ioconf.c"
#define LOG_TICK_WIDTH	16	//The width of progress bar ticks picture
int log_cnt = 1;
void print_logo_tick(void)
{
#ifdef PROGRESS_BAR
	if(!disableoutput)	//abort when the logo is not initialized, or video output is enable
		return;
	int i;
	/* dev scan maybe call print_logo_tick() more times */
	int length = (VIDEO_VISIBLE_COLS / (LOG_CNT - 1)) * (log_cnt == LOG_CNT ? (LOG_CNT - 1) : log_cnt++);
	video_display_bitmap (logo_data_left, 0, AT_END);
	for (i = 1; i < length / LOG_TICK_WIDTH; i++) {
		video_display_bitmap (logo_data_set1, LOG_TICK_WIDTH * i, AT_END);
	}
	video_display_bitmap (logo_data_mid, LOG_TICK_WIDTH * i++, AT_END);
	for (; i < VIDEO_VISIBLE_COLS / LOG_TICK_WIDTH; i++) {
		video_display_bitmap (logo_data_set0, LOG_TICK_WIDTH * i, AT_END);
	}

	video_display_bitmap (logo_data_right, AT_END, AT_END);
#endif
}

void logo_occupy_scr(int ops)
{
#ifdef PROGRESS_BAR
	disableoutput = ops;
#endif
